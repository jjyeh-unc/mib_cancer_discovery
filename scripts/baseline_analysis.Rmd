...
RNA_meta_MIB <- m_rna[grepl("^PDX$", m_rna$Type),]
RNA_meta_MIB <- RNA_meta_MIB[match(m2$linems, RNA_meta_MIB$Experiment),]
RNA_meta_MIB <- RNA_meta_MIB[!is.na(RNA_meta_MIB$Sample),]
line_key <- aggregate(line ~ line_trunc, unique(data.frame(line = m2$line2, line_trunc = m2$line)), paste)
RNA_meta_MIB2 <- RNA_meta_MIB %>% left_join(line_key, by = ("line"))

RNA_ex_MIB <- RNA_ex[,match(RNA_meta_MIB$Sample, names(RNA_ex))]
RNA_ex_ENSG_MIB <- RNA_ex_ENSG[,match(RNA_meta_MIB$Sample, names(RNA_ex_ENSG))]
...
col_ha1 = columnAnnotation(
  #SSC = ca4,
  line = as.matrix(RNA_meta_MIB2$line_trunc),
...
m_prot <- m2[match(RNA_meta_MIB$Experiment, m2$linems),]
...
ID_order <- unique(m_RNA_line$line_trunc) #define order of samples

m_RNA_line <- m_RNA_line %>% 
  group_by(line_trunc) %>% 
  summarise_all(concat_unique)
m_RNA_line <- m_RNA_line[match(ID_order, m_RNA_line$line_trunc),]

#match protein and RNA expression
prot <- d.log2[,match(m_prot$ID ,colnames(d.log2))]
colnames(prot) <- m_prot$linems

RNA <- RNA_ex[,match(RNA_meta_MIB2$Sample ,colnames(RNA_ex))]
...
c_RNA$line <- RNA_meta_MIB2$line_trunc
...
c_RNA <- c_RNA[match(m_RNA_line$line_trunc, c_RNA$line),]
...
png("plots/supp4d.png",width=8,height=8,units="in",res=500)
classical8
invisible(dev.off())

png("plots/supp4e.png",width=8,height=8,units="in",res=500)
basal8
invisible(dev.off())
