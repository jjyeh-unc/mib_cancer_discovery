#parsing multiple excel sheets
fn_multiplesheets <- function(fname) {
...
#impute functions based on number of replicates
impute3 <- function(d)
{d [d  == 0] <- NA
d$countNA <- rowSums(is.na(d[1:3]))
d[with(d, d$countNA >1),] <- NA
k <- which(is.na(d[1:3]), arr.ind=TRUE)
d[k] <- rowMeans(d[1:3], na.rm=TRUE)[k[,1]]
d <- d[,1:3]
}

impute4 <- function(d)
{d [d  == 0] <- NA
d$countNA <- rowSums(is.na(d[1:4]))
d[with(d, d$countNA >1),] <- NA
k <- which(is.na(d[1:4]), arr.ind=TRUE)
d[k] <- rowMeans(d[1:4], na.rm=TRUE)[k[,1]]
d <- d[,1:4]
}

impute5 <- function(d)
{d [d  == 0] <- NA
d$countNA <- rowSums(is.na(d[1:5]))
d[with(d, d$countNA >2),] <- NA
k <- which(is.na(d[1:5]), arr.ind=TRUE)
d[k] <- rowMeans(d[1:5], na.rm=TRUE)[k[,1]]
d <- d[,1:5]
}

impute6 <- function(d)
{d [d  == 0] <- NA
d$countNA <- rowSums(is.na(d[1:6]))
d[with(d, d$countNA >2),] <- NA
k <- which(is.na(d[1:6]), arr.ind=TRUE)
d[k] <- rowMeans(d[1:6], na.rm=TRUE)[k[,1]]
d <- d[,1:6]
}

impute9 <- function(d)
{d [d  == 0] <- NA
d$countNA <- rowSums(is.na(d[1:9]))
d[with(d, d$countNA >4),] <- NA
k <- which(is.na(d[1:9]), arr.ind=TRUE)
d[k] <- rowMeans(d[1:9], na.rm=TRUE)[k[,1]]
d <- d[,1:9]
}

impute12 <- function(d)
{d [d  == 0] <- NA
d$countNA <- rowSums(is.na(d[1:12]))
d[with(d, d$countNA >5),] <- NA
k <- which(is.na(d[1:12]), arr.ind=TRUE)
d[k] <- rowMeans(d[1:12], na.rm=TRUE)[k[,1]]
d <- d[,1:12]
}

impute14 <- function(d)
{d [d  == 0] <- NA
d$countNA <- rowSums(is.na(d[1:14]))
d[with(d, d$countNA >6),] <- NA
k <- which(is.na(d[1:14]), arr.ind=TRUE)
d[k] <- rowMeans(d[1:14], na.rm=TRUE)[k[,1]]
d <- d[,1:14]
}


##function for median without NA
medianWithoutNA<-function(x) {
  median(x[which(!is.na(x))])
}

#consensus cluster plus custom colors function
ccRun <- function( d=d,
                   maxK=NULL,
                   repCount=NULL,
                                   ...
                                   weightsItem = NULL, weightsFeature = NULL, verbose = F, corUse = "everything") 
{
  ...
  colorList = list()
  colorM = rbind()
  thisPal <- c("#6A3D9A", "#f4cc03", "#B15928", "#A6CEE3", "#1F78B4", "#B2DF8A", "#33A02C", 
                        "#FB9A99", "#E31A1C", "#FDBF6F", "#FF7F00", "#CAB2D6", 
                        "#FFFF99", "#bd18ea", "#2ef4ca", 
                        "#f4cced", "#05188a", "#e5a25a", "#06f106", 
